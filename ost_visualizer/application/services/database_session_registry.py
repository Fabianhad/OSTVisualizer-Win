...
class DatabaseSessionRegistry:
    def __init__(self) -> None:
        ...
        self._lock_tokens: dict[tuple[str, ResourceRef], str] = {}
    ...
    def register_lock(
        ...
        with self._lock:
            self._lock_tokens[(database_id, resource)] = lock_token

    def remove_lock(self, database_id: str, resource: ResourceRef) -> None:
        with self._lock:
            self._lock_tokens.pop((database_id, resource), None)
    ...
    def lock_tokens(
        ...
        with self._lock:
            return tuple(
                self._lock_tokens[(database_id, resource)]
                for resource in resources
                if (database_id, resource) in self._lock_tokens
            )
