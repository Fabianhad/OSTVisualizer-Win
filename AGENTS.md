...
After any release-facing update, version bump, user-visible fix, or stability improvement, update `CHANGELOG.md` for the next release, not the last released version. If the current `CHANGELOG.md` only contains notes for an already-launched version and the working tree is starting a new update cycle, replace it with a fresh section for the next version (for example, `## 1.2.2.1 - Unreleased`) and include only changes made since the previous release. Do not carry forward fixes from already-launched versions, and do not edit a released section except to correct factual errors.
