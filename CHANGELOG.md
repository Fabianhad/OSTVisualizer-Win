...
- Added first-class Microsoft SQL Server databases with encrypted Windows or SQL authentication, Windows Credential Manager secrets, saved descriptors, compatible external-database adoption, direct current-schema creation, and multi-user desktop sessions with presence, expiring resource locks, optimistic concurrency, transactional change records, reconnect-safe delta polling, and targeted UI synchronization. Microsoft Access keeps its existing workflow and does not start collaboration services.
