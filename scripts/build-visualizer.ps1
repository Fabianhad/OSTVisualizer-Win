...
$StartTime = Get-Date
Write-Host "OST Visualizer - Desktop Release Build" -ForegroundColor Cyan
Write-Host "  Parallel Jobs: $CpuCores" -ForegroundColor Green
Write-Host "Building desktop app..." -ForegroundColor Cyan

& nuitka @NuitkaArgs
if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: Nuitka failed while building the desktop app." -ForegroundColor Red
    exit $LASTEXITCODE
}

$Duration = (Get-Date) - $StartTime
Write-Host "Desktop build completed in $($Duration.ToString('hh\:mm\:ss'))" -ForegroundColor Green
